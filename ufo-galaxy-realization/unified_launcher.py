...
class ServiceManager:
    ...
    async def watchdog_loop(self):
        ...
        # 初始化状态机集成
        from system_integration.state_machine_ui_integration import SystemStateMachine
        self.state_machine = SystemStateMachine()
        self.state_machine.wakeup_signal.connect(
            lambda data: asyncio.create_task(self.broadcast_signal(data))
        )
            ...
            for name, service in self.services.items():
                if service.status == "running" and service.process:
                    ...
                    if return_code is not None:
                        logger.warning(f"服务 {name} 意外退出 (代码: {return_code})，正在尝试重启...")
                        service.status = "restarting"
                        # 简单的重启策略：等待 2 秒后重启
                        await asyncio.sleep(2)
                        # 重新构建启动命令（这里简化处理，假设之前的 command 仍然有效，但在实际中可能需要存储 command）
                        # 由于 ServiceInfo 没有存储 command，我们需要在 start_service 中存储它
                        # 这里暂时只打印日志，提示需要重启
                        logger.error(f"服务 {name} 需要手动重启 (自动重启逻辑需完善 command 存储)")
