...
class TopologyAwareConstellationClient(ConstellationClient if CONSTELLATION_AVAILABLE else object):
    ...
    async def _send_task_to_node(self, node_id: str, task: Dict[str, Any]) -> str:
        ...
        # 尝试通过 device_manager 发送任务
        if hasattr(self, 'device_manager') and self.device_manager:
            try:
                # 假设 device_manager 有 send_command 或类似方法
                # 这里我们模拟发送一个 ExecuteTask 命令
                command = {
                    "type": "execute_task",
                    "payload": task
                }
                # 注意：这里假设了 device_manager 的接口，实际需根据 UFO 源码调整
                # await self.device_manager.send_to_device(node_id, command)
                logger.info(f"✅ Task successfully dispatched to {node_id}")
            except Exception as e:
                logger.error(f"❌ Failed to dispatch task to {node_id}: {e}")
                raise
        else:
            # 如果没有 device_manager (例如在测试模式)，则仅记录
            logger.warning(f"⚠️  DeviceManager not available, task dispatch simulated for {node_id}")
